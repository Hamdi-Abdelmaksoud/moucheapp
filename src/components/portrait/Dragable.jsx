...
export default function Dragable({ pics }) {
  const cardPosition=['absolute top-15 left-[33%] sm:left-[43%] rotate-[-5deg]',
  ...
  return (
    <div className="w-full h-700 overflow-hidden">
  ...
  The fly sees the unseen
      ...
      {pics.map((pic, index) => (
          <DraggableCardBody key={index} className={cardPosition[index]}>
            <img
              src={pic.url}
              alt={pic.title}
